# app.py
# ------------------------------------------------------------
# 박물관 해설 서비스 예약 시스템 (Streamlit 1-page)
# - 왼쪽 상단: 박물관 이미지/정보
# - 왼쪽 하단: 해설 예약 폼
# - 오른쪽: 챗봇 (최근 5개만 표시 + 입력 하단)
# - 챗봇에서 "예약해줘" 등 요청 시 → 예약 초안(draft)을 자동으로 채움(폼에 반영)
#
# 실행:
...
def build_context_for_llm() -> str:
    ...
    for s in services:
        ...
        if "price_per_person" in s: #1인 당 금액인 경우
            base += f" | 1인 {currency(s['price_per_person'])}" #기본 정보에 1인당 금액 정보 추가
        if "price_per_group" in s:  #그룹 당 금액인 경우
            base += f" | 1그룹 {currency(s['price_per_group'])}(최대 4인 예시)" #기본 정보에 그룹당 금액 정보 추가
...
def chat_and_maybe_fill_draft(user_message: str) -> tuple[str, dict]:
    ...
    if getattr(msg, "tool_calls", None):  #응답 msg에 tool_calls가 있다면
...
def render_chat_bubbles(max_visible: int = 5):
    ...
    if len(chat) > max_visible: #최대 갯수 보다 채팅 갯수가 많은 경우
        ...
        with st.expander(f"이전 대화 보기 ({len(old)}개)", expanded=False): #이전 채팅 영역 만들기
...
def main():
    ...
    with left:
        ...
        # 날짜 선택(다음 14일)
        days = get_next_days(14) #오늘 기준 이후 14일치 날짜 객체 리스트
        ...
        if draft["service_id"]: #선택항목이 있는 경우 선택 예시 정보 와 선택 회차, 인원 정보 항목 데이터 셋팅
            ...
            if svc:
                ...
                if "price_per_person" in svc:
                    price_hint = f"1인 {currency(svc['price_per_person'])}" # currency : 금액 포맷팅
                elif "price_per_group" in svc:
                    price_hint = f"1그룹 {currency(svc['price_per_group'])}(최대 4인 예시)"
                    is_group = True
        ...
        with t1:
            draft["time"] = st.selectbox(
                "회차(시간)",
                options=["선택하세요"] + times,
                index=0 if draft["time"] not in times else (times.index(draft["time"]) + 1), #시간 정보가 없으면 선택하세요 지정 있으면 인덱스+1 해서 선택
            )
            if draft["time"] == "선택하세요":
        ...
        total = compute_price(draft)  #총 금액 가져오기
        st.markdown(f"### 예상 결제금액: **{currency(total)}**")
            ...
            if st.button("예약 저장", use_container_width=True): #  use_container_width=True ->버튼을 부모 컨테이너 너비 맞춤 / 버튼 출력 클릭 시 if 실행
                ...
                if not ok: #유효성 검사 false 경우
                    ...
                    summary = (
                        ...
                        f"- 시간: {saved['time']}\n"
                        f"- 인원: {saved['people']}\n"
                        f"- 금액: {currency(saved['total_price'])}"
                    )
                    st.session_state.chat.append({"role": "assistant", "content": summary}) #예약 완료 정보 채팅에 추가
